...
const MyOrders = () => {
    ...
    return(
        <div className={oscss.container}>
            <Header/>
                <h1 className={oscss.MOH1}>MY ORDERS</h1>
                {
                    arr.map((order)=>{
                        return(
                            <div className={oscss.orderContainer}>
                                <p><b>ORDER </b>{order.invoice_id}</p>
                                <p><b>STATUS </b>{order.status}</p>
                                <p><b>DATE ORDERED: </b>{order.orderdate.slice(0,10)}</p>
                                <Link to="/orderstatus"
                                    onClick={()=>{
                                ...
                                ><button className={oscss.viewButton}>View Order<BsBoxArrowUpRight/></button></Link>
                            </div>
                        )
                    })
                }
